...
**SMP Whitepaper v0.3 — the Engram increment.** An early specification, honestly labeled: offered for review and separately anchored for provenance and priority — not a finished 1.0. It ran in shadow mode (measuring, not steering) until 2026-08-25; since then the reference installation lets it steer ahead of this paper's own gates — see the status update in §10.
...
- **Shadow mode (at anchoring).** Engram was in Phase 1: every night, after REM, the observer computes `S` and `R` for every memory and writes watchdog metrics. **It steered nothing.** This is a deliberate shadow deployment — the model runs in parallel and is measured before it is ever allowed to act.
...
- **Status update 2026-09-23 — the reference installation now lets Engram steer, ahead of this paper's own gates.** Since 2026-08-25 two things depend on `R`: the cascade moves a daily entry onward when its retrievability falls below a threshold (with an age floor as a safety net) instead of by age alone, and entries marked dormant are damped in the recall ranking by a factor of 0.9 — never excluded. The second departs from the hard invariant of §6 as written ("`S` and `R` never enter the recall score"), and neither was preceded by the full release conditions of §11. It is disclosed here rather than smoothed over. **Review closes on 2026-10-07**, on a measurement rather than a preference: do damped entries still surface when they are needed — are dormant entries retrieved and reanimated — or does the damping start a forgetting loop (damped → surfaces less → retrieved less → stays damped), the mirror image of the runaway §6 forecloses? If they return, §6 is restated to allow bounded damping that never excludes; if not, the damping is withdrawn. The paragraphs above and below describe the state when this paper was anchored.
- **Not yet steering (at anchoring).** No memory had been compacted by Engram. Phase 2 (a double-look: compact only when both the age-based rule and the relative `S/R` rule agree) and Phase 3 (`S/R` leads; time is only the `t` in the curve) follow only after the shadow metrics stay stable under observation.
...
We claim no novelty of mechanism — only the honesty of the fit and the discipline of the rollout. When this paper was anchored, the model ran in the dark, weighing and not yet touching, meant to step into the light on measured evidence, not on a promise — where it stepped out early, §10 says so. A protocol about proving one's own past should grow the way it remembers: the old spec frozen and anchored, this increment separately anchored and referring back, and the reachable index kept live. This paper is that increment, offered in draft to the partner who holds the keys.
