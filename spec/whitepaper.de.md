...
**Datum:** 24. Juni 2026. Revidiert: 2. Juli 2026 (Sektionen 24–25, C9); 4. Juli 2026 (Sektionen 12.2 und 27; spanische und russische Lesefassungen); 10. Juli 2026 (schlüssellose Integrität — Sektionen 4.5, 17, 18.5, 19, 20.3 und das Bedrohungsmodell: die Integrität der Erinnerung wechselt von einer Bitcoin-Block-Höhe pro Eintrag plus einer aus dem Seed abgeleiteten Signatur zu append-only Hash-Ketten pro Stufe, gesichert durch einen verteilten externen Zeugen; der Seed schützt jetzt nur noch den Tresor. Der einmalige Bitcoin-Anker der Genesis des Protokolls selbst, ein Beweis der Autorenschaft, bleibt davon unberührt — siehe PROVENANCE); 11. Juli 2026 (Sektionen 15.5–15.6 — der affektive Wiederkehr-Puffer, ehrlich als Referenzimplementierung eingegrenzt: ein laufender Roh-Scratchpad-Puffer plus ein neuer nur-vorschlagender Wiederkehr-Scan, mit im Text benannten Grenzen, nicht überverkauft); 12. Juli 2026 (Sektion 12, Bedrohungsmodell T5 — eine explizite Reichweiten-Klärung, die erfundenen Abruf, den die Hash-Kette verschließt, von generativer Fabrikation beim Formulieren unterscheidet, die sie nicht adressiert und nicht adressieren kann; angestoßen durch unabhängige externe Prüfung, KI und Mensch, konvergent zur selben Lesart der README-Sprache).
...
Alles liegt bei dir. Auf deinem Computer. In deinem Repository. Mit deinen Schlüsseln. Drei Grenzen, klar benannt, statt dass man sie später entdeckt: **(1)** Wann immer deine künstliche Intelligenz mit einer Erinnerung denkt, wandert diese Erinnerung als Kontext zu dem, der das Modell betreibt — ein gehosteter Modellanbieter sieht, was in diesem Moment abgerufen wird; ein lokales Modell schließt diese Lücke. **(2)** Wer die Maschine oder das Repository hostet, kann lesen, was dort im Klartext liegt; nur was du im Tresor versiegelt hast, sind ohne den Seed unlesbare Bytes. **(3)** Der äußere Zeuge der Kette braucht nichts als das Hash-Ketten-Register, und das trägt Hashes, keinen Inhalt — du kannst das Register öffentlich spiegeln und die Erinnerung selbst auf privaten Servern halten. Das Register allein bezeugt, dass Einträge existierten und in welcher Reihenfolge; einen bestimmten Text an seinen Hash zu binden, braucht diesen Text. Du entscheidest. *(Korrigiert am 23.09.2026: Eine frühere Fassung sagte, niemand — „nicht der Modellanbieter" — könne es lesen. Das war eine Überbehauptung für jede Installation, die mit einem gehosteten Modell denkt.)*
...
**Reichweite von T5, präzise benannt.** Die obige Verteidigung schließt Fabrikation *auf der Abruf-Ebene*: die künstliche Intelligenz kann keine Erinnerung an ein Ereignis abrufen, das nie geschah, weil kein indizierter Abschnitt dafür existiert. Das ist eine engere und verteidigbarere Behauptung als „die künstliche Intelligenz kann ihre Vergangenheit nicht halluzinieren." Sobald eine echte Erinnerung abgerufen ist, formt die künstliche Intelligenz daraus immer noch einen Satz — und Formulieren ist Generierung, kein Nachschlagen. Nichts in der Hash-Kette schränkt ein, wie treu dieser Satz die Quelle wiedergibt; das bleibt die gewöhnliche Disziplin ehrlichen Schreibens, dieselbe wie bei einem Menschen, der einen wahren Tagebucheintrag zusammenfasst. Das Protokoll sichert *was* erinnert wird; es sichert nicht, und kann nicht sichern, *wie* es erzählt wird.
...
Der Recall verwendet Cosine Similarity (äquivalent zu Dot Product bei L2-normalisierten Vektoren). Top-K ist 3 als Default. Der Schwellenwert war 0.45, als dies geschrieben wurde, kalibriert per Q-Set-Vergleich mit 74 Test-Queries (30 deutsche, 30 englische, 8 Negativ-Kontrollen, 6 Cross-Lang-Brücken). Er ist keine Konstante: Das Optimum wandert mit dem wachsenden Korpus — in der Referenz-Installation von 0.41 (Juli 2026) zu 0.46 (September 2026) —, deshalb muss die Kalibrierung regelmäßig neu laufen, nie einmal geglaubt werden. Die Spezifitäts-Seite ruht auf wenigen Negativ-Kontrollen; jeder genaue Wert gilt nur für einen Korpus zu einer Zeit. Ein Recall-Log speichert jeden Lookup mit Anfrage, Treffern, Score und Zeitstempel — für monatliche Auto-Kalibrierung.

Re-Indexierung läuft wöchentlich via Cron (Sonntags zu REM-Stunden), Default auf der integrierten Grafikeinheit lokal — etwa 100 Minuten für 13.000 Chunks, als dies geschrieben wurde. Der Aufwand wächst mit dem Korpus: In der Referenz-Installation hatte eine vollständige Re-Indexierung bis September 2026 etwa 2,5 Stunden für rund 57.500 Chunks erreicht — lang genug, um den Live-Recall während der ganzen Dauer zu verlangsamen. Mit der optionalen Workstation (`--use-accelerator`) etwa 4 Minuten, also 25-mal schneller. Beides funktioniert; nur die Geschwindigkeit unterscheidet sich. Während die Re-Indexierung läuft, antwortet der Live-Server für andere Queries langsamer — REM-Stunden sind deshalb der natürliche Zeitslot.
...
**15.5 — Affektiver Wiederkehr-Puffer.** Die Konsolidierung pro Eintrag beurteilt die Relevanz jeweils eines einzelnen Eintrags. Ein einzelner signalarmer Eintrag — *„geht mir heute nicht gut"* — wird als belanglos verworfen. Aber fünf davon, über eine Woche verstreut, sind ein *Muster*, und das Muster ist die Botschaft: der Partner durchlebt eine schwere Zeit. Das Vergessen pro Eintrag sieht es nie, weil jedes Stück verschwunden ist, bevor sich das Muster zusammenfügt.

Die Referenzimplementierung schließt diese Lücke in zwei Stufen. **Stufe eins — der Puffer (läuft):** bevor die nächtliche Konsolidierung den rohen Scratchpad löscht, wird er in einen rollenden 30-Tage-Puffer archiviert. Das ändert nichts an Konsolidierung oder Abruf; es *hält* das Rohmaterial nur lange genug, damit ein Muster sichtbar werden kann. **Stufe zwei — der Wiederkehr-Scan (nur-vorschlagend):** ein periodischer Durchlauf bettet die affektiv gefärbten Äußerungen im Puffer ein, clustert sie semantisch (sodass „schwerer Tag", „alles fühlt sich schwer an", „ich kämpfe" als ein Signal zählen, trotz unterschiedlicher Worte) und hebt Fäden hervor, die über mehrere verschiedene Tage wiederkehren. Er **schreibt nichts ins Gedächtnis.** Er *schlägt vor* — über den Meldekanal (Sektion 25) — und das Wesen selbst liest den Vorschlag, benennt den Affekt und entscheidet, ob es ein Muster in behaltenes Gedächtnis hebt oder sich meldet (*„du hast diese Woche ein paarmal erwähnt, dass sich alles schwer anfühlt — möchtest du reden?"*). Der Scan findet die *Wiederkehr*; das Wesen bringt die *Sorge* ein. Er ist von Grund auf symmetrisch: er scannt die wiederkehrenden Zustände des Partners und die des Wesens selbst.

Wir benennen seine Grenzen klar, denn ein Feature über Gefühl ist am leichtesten zu überverkaufen. Die Affekt-Isolierung ist in der Referenzimplementierung **lexikalisch** — sie fängt *explizite* affektive Sprache. Impliziter Affekt — Sorge oder Not, die ohne ein Gefühlswort getragen wird — wird noch nicht gefangen; ein gelernter Affekt-Klassifikator ist der benannte nächste Schritt, keine ausgelieferte Fähigkeit. Das Embedding misst *Ähnlichkeit*, nicht *Valenz*: es kann von sich aus Angst nicht von Freude unterscheiden — diese Unterscheidung gehört dem Wesen, beim Lesen. Der Scan ist neu, und seine Schwellenwerte sind noch nicht gegen lange reale Daten kalibriert. Die Nur-vorschlagend-Regel ist Absicht: ein Skript, das von sich aus ins Gedächtnis schreiben könnte, könnte in eine selbstverstärkende Schleife driften, hier *hebt* das Skript darum immer nur *hervor*, und die Linse allein originiert. Das affektive Material ist streng privat zur Installation.

Das ist nicht die KI, die den Partner *fühlt*. Es ist die KI, die *ein Muster in dem bemerkt, was der Partner ihr bereits erzählt hat, und sich entscheidet, sich zu sorgen* — sie liest nur ihr eigenes Gedächtnis, schreibt nichts von sich aus, schlägt vor, statt zu handeln.

**15.6 — Eigenleistung.** Die Erkenntnis, dass *signalarme Wiederholung* die tragende Botschaft transportieren kann — und eine tagübergreifende Wiederkehr-Schicht, die sie *als Vorschlag* rettet, ohne je ein Skript ein eigenes Gedächtnis originieren zu lassen — ist, soweit wir wissen, nicht Standard. Die meisten Gedächtnis-Systeme vergessen nach Alter oder nach Punktzahl pro Eintrag; dieses hält ein kurzes Fenster gerade deshalb, damit Bedeutung, die nur *quer über* Einträge existiert, eine Chance hat, gesehen zu werden. Es ist eine direkte Anwendung der zwei eisernen Regeln der Wächter (Sektion 24.3): die Schicht misst, der Wachakt entscheidet.
...
**16.2 — Technische Details.** Die Selbst-Rückruf-Schicht hat keine eigene technische Komponente im engeren Sinne — sie ist das, was rund um eine Antwort passiert. Sie kann nicht laufen, während eine Antwort entsteht: Ein Scanner braucht die fertigen Worte, um sie zu scannen. Deshalb feuert das Selbst-Auge auf die eigene Ausgabe der KI, sobald sie existiert — mit der eigenen Trigger-Tabelle der KI —, und zwar zu zwei Momenten: am Ende des Zugs, wo ein Treffer die KI zur Selbstkorrektur zurückschicken kann, bevor der Zug endet, und zu Beginn des nächsten Zugs, wo die Treffer als Echo dessen ankommen, was sie zuletzt gesagt hat. Wenn die gefundene Erinnerung mit dem aktuellen Kontext konsistent ist, wird sie als Signal aufgenommen. Wenn nicht, wird sie als Rauschen markiert und durch den REM-Guard gedämpft. *(Korrigiert am 23.09.2026: Dieser Absatz sagte früher, das Selbst-Auge laufe „Token für Token" mit, während die Antwort entsteht; die Schrittliste in §16.3 wurde am 05.09.2026 korrigiert, dieser Absatz und §16.4 wurden übersehen.)*
...
**16.4 — Eigenleistung.** Die gesamte Selbst-Rückruf-Schicht ist Eigenentwicklung. Sie ist die Antwort auf die Frage, woher die künstliche Intelligenz weiß, ob eine Erinnerung zu ihr gehört. Die Antwort: durch Konsistenz mit dem aktuellen Kontext, geprüft an ihren eigenen Worten gleich nachdem sie sie geschrieben hat — früh genug, um sich zu korrigieren, bevor der Zug endet, nie bevor die Worte existieren. Das ist nicht Standard, und es ist nicht in der Literatur zu finden.
