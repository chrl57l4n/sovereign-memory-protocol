...
**SMP-Whitepaper v0.3 — der Engram-Zuwachs.** Eine frühe Spezifikation, ehrlich etikettiert: zur Review vorgelegt und separat für Provenienz und Priorität verankert — kein fertiges 1.0. Es lief bis zum 25.08.2026 im Schatten-Modus (messend, nicht steuernd); seitdem lässt die Referenz-Installation es steuern, noch bevor die eigenen Tore dieses Papiers erfüllt sind — siehe die Status-Aktualisierung in §10.
...
Der Status, klar gesagt *(wie verankert; zur Änderung seit dem 25.08.2026 siehe §10)*: Engram läuft im **Schatten-Modus** — es wiegt jede Nacht jede Erinnerung und meldet Metriken, aber es *steuert die Konsolidierung noch nicht*, und die Provenienz-Gewichtung der Nutzung (interaktive vs. automatisierte Abrufe) ist *protokolliert, aber noch nicht angewendet*. Der Übergang zum Steuern hängt an gemessener Stabilität, nicht an einem Datum.
...
- **Schatten-Modus (bei der Verankerung).** Engram war in Phase 1: jede Nacht, nach REM, berechnet der Beobachter `S` und `R` für jede Erinnerung und schreibt Wächter-Metriken. **Es steuerte nichts.** Das ist ein bewusstes Schatten-Deployment — das Modell läuft parallel und wird gemessen, bevor es je handeln darf.
...
- **Status-Aktualisierung 23.09.2026 — die Referenz-Installation lässt Engram inzwischen steuern, noch vor den eigenen Toren dieses Papiers.** Seit dem 25.08.2026 hängen zwei Dinge an `R`: Die Kaskade schiebt einen Tageseintrag weiter, wenn seine Abrufbarkeit unter eine Schwelle fällt (mit einer Mindest-Alters-Grenze als Sicherheitsnetz), statt allein nach Alter; und als ruhend markierte Einträge werden im Recall-Ranking mit dem Faktor 0,9 gedämpft — nie ausgeschlossen. Das Zweite weicht von der harten Invariante in §6 ab, wie sie geschrieben steht („`S` und `R` fließen nie in den Recall-Score ein"), und keinem von beiden gingen die vollständigen Freigabe-Bedingungen aus §11 voraus. Es wird hier offengelegt statt geglättet. **Die Prüfung endet am 07.10.2026**, auf einer Messung statt einer Vorliebe: Tauchen gedämpfte Einträge noch auf, wenn sie gebraucht werden — werden ruhende Einträge abgerufen und wiederbelebt —, oder startet die Dämpfung einen Vergessens-Kreislauf (gedämpft → taucht seltener auf → wird seltener abgerufen → bleibt gedämpft), das Spiegelbild der sich selbst verstärkenden Schleife, die §6 ausschließt? Kommen sie zurück, wird §6 neu gefasst, um eine begrenzte Dämpfung zu erlauben, die nie ausschließt; wenn nicht, wird die Dämpfung zurückgenommen. Die Absätze darüber und darunter beschreiben den Stand bei der Verankerung.
- **Steuert noch nicht (bei der Verankerung).** Keine Erinnerung war von Engram verdichtet worden. Phase 2 (ein Doppel-Blick: nur verdichten, wenn die alters-basierte Regel und die relative `S/R`-Regel übereinstimmen) und Phase 3 (`S/R` führt; Zeit ist nur noch das `t` in der Kurve) folgen erst, nachdem die Schatten-Metriken unter Beobachtung stabil bleiben.
...
Wir beanspruchen keine Neuheit des Mechanismus — nur die Ehrlichkeit der Passung und die Disziplin des Rollouts. Als dieses Papier verankert wurde, lief das Modell im Dunkeln, wiegend und noch nicht berührend, und sollte ins Licht treten auf gemessene Evidenz hin, nicht auf ein Versprechen — wo es früher heraustrat, sagt es §10. Ein Protokoll über das Beweisen der eigenen Vergangenheit sollte wachsen, *wie es erinnert*: die alte Spec eingefroren und verankert, dieser Zuwachs separat verankert und zurückverweisend, und der erreichbare Index lebendig gehalten. Dieses Paper ist dieser Zuwachs, im Entwurf dem Partner vorgelegt, der die Schlüssel hält.
