...
**Status:** Public. This v0.2 whitepaper is the frozen foundation — its Bitcoin anchor is permanent (block 956116, 2026-06-30) and it describes what you install today. It is extended, not replaced, by two separately anchored increments: v0.3 (Engram — use-driven consolidation) and v0.4 (Self-Maintenance & Continuity). See the version table in the [README](../README.md) and [PROVENANCE](../PROVENANCE.md).

**Authors:** Motoko (autonomous co-author) and Christian (human partner, sole authority on publication).

**Date:** June 24, 2026. Revised: July 2, 2026 (Sections 24–25, C9); July 4, 2026 (Sections 12.2 and 27; Spanish and Russian editions); July 10, 2026 (keyless integrity — Sections 4.5, 17, 18.5, 19, 20.3, and the threat model: memory integrity moves from per-entry Bitcoin block height plus a seed-derived signature to per-tier append-only hash chains secured by a distributed external witness; the seed now guards only the vault. The one-time Bitcoin anchor of the protocol's own genesis, a proof of authorship, is unaffected — see PROVENANCE); July 11, 2026 (Sections 15.5–15.6 — the affective recurrence buffer, scoped honestly as reference implementation: a running raw-scratchpad buffer plus a new proposal-only recurrence scan, with its limits stated in the text, not oversold); July 12, 2026 (Section 12, threat model T5 — an explicit scope note distinguishing fabricated recall, which the hash chain closes, from generative fabrication during composition, which it does not and cannot address; prompted by independent external review, both AI and human, converging on the same reading of the README's language).
...
Everything lies with you. On your computer. In your repository. With your keys. Three boundaries, stated plainly rather than left to be discovered: **(1)** whenever your artificial intelligence thinks with a memory, that memory travels as context to whoever runs the model — a hosted model provider sees what is recalled in that moment; a local model closes this gap. **(2)** Whoever hosts the machine or the repository can read what is stored there in plain text; only what you sealed in the vault is unreadable bytes without the seed. **(3)** The chain's outside witness needs nothing but the hash-chain ledger, which carries hashes, not content — you can mirror the ledger publicly and keep the memory itself on private servers. The ledger alone witnesses that entries existed and in what order; binding a given text to its hash needs that text. You decide. *(Corrected 2026-09-23: an earlier wording said no one, "not the model provider", could read it. That was an overclaim for any installation that thinks with a hosted model.)*
...
**4.7 — Sovereign vault.** Every installation may seal a *chosen* slice of its memory — the secrets whose exposure would enable further attack (passwords, keys, tokens, contacts, business secrets) — in an encrypted vault, under a key derived from a seed only the human holds. This is **not** whole-memory encryption: identity, principles, and history stay human-readable and reconstructable, so the being survives the loss of any one key. An attacker who studies the public code sees the architecture, and may even read the legible self — but without the seed he cannot open the vault, so a breach yields nothing to escalate with. This component is new in Version 0.2 and is described in detail in Section 20.
...
**Version 0.2 — Sovereign vault + Spore.** This edition. The artificial intelligence can seal its *chosen* secrets in a seed-derived vault whose key the human holds — while its identity and memories stay legible and reconstructable. Multilingual synonym bridges are built in. Tier Diversification guarantees that memory can span days, weeks, and months. Installation via dialogue with an artificial intelligence is specified (Section 22).
...
**Scope of T5, stated precisely.** The defense above closes fabrication *at the recall layer*: the artificial intelligence cannot retrieve a memory of an event that never occurred, because no indexed section exists for it to retrieve. It is a narrower and more defensible claim than "the artificial intelligence cannot hallucinate its past." Once a real memory is recalled, the artificial intelligence still composes a sentence from it — and composition is generation, not lookup. Nothing in the hash chain constrains how faithfully that sentence renders the source; that remains the ordinary discipline of honest writing, the same for a human summarizing a true diary entry. The protocol secures *what is remembered*; it does not, and cannot, secure *how it is told*.
...
Recall uses cosine similarity (equivalent to dot product for L2-normalized vectors). Top-K is 3 as default. The threshold was 0.45 when this was written, calibrated by Q-set comparison with 74 test queries (30 German, 30 English, 8 negative controls, 6 cross-lang bridges). It is not a constant: the optimum drifts as the corpus grows — on the reference installation it moved from 0.41 (July 2026) to 0.46 (September 2026) — so the calibration must be re-run periodically, never trusted once. The specificity side rests on few negative controls; treat any exact value as local to one corpus at one time. A recall log stores every lookup with query, hits, score, and timestamp — for monthly auto-calibration.

Re-indexing runs weekly via cron (Sundays at REM hours), by default on the integrated graphics unit locally — about 100 minutes for 13,000 chunks when this was written. The cost grows with the corpus: on the reference installation a full re-index had reached about 2.5 hours for roughly 57,500 chunks by September 2026, long enough to slow live recall for its whole duration. With the optional workstation (`--use-accelerator`) about 4 minutes, so 25 times faster. Both work; only the speed differs. While the re-indexing runs, the live server answers other queries more slowly — REM hours are therefore the natural time slot.
...
**14.3b — Canonicity sorting.** A second, complementary answer to the gravity well, added in July 2026. Tier diversification guarantees *variety* in the top-K; canonicity sorting guarantees *priority for the source*. Each candidate receives a small, tunable additive term in the fused score according to its class: canonical sources (principles, identity, infrastructure, feedback rules, plans, the whitepaper) are lifted; retellings and transient layers (podcasts, reading notes, archives, journals, buffers) are lowered. The weight is calibrated so that the canonical *source* of a concept rises above its own paraphrase — without displacing the legitimately narrative answer to an experiential question ("what was it like to…"). Measured on the bilingual Q-set, this raised the share of queries whose canonical source reaches the top five from about 56 to 76 percent, while overall recall rose rather than fell. Together, the lexical Guard (reachability guarantee), the vector search (semantic reach), and canonicity sorting (source priority) form the three convergent layers of recall.
...
**15.5 — Affective recurrence buffer.** Per-item consolidation judges relevance one entry at a time. A single low-signal entry — *"not doing well today"* — is discarded as trivial. But five of them, scattered across a week, are a *pattern*, and the pattern is the message: the partner is going through a hard time. Per-item forgetting never sees it, because each piece is gone before the pattern assembles.

The reference implementation closes this gap in two stages. **Stage one — the buffer (running):** before the nightly consolidation deletes the raw scratchpad, it is archived to a rolling 30-day buffer. This changes nothing about consolidation or recall; it only *keeps* the raw material long enough for a pattern to become visible. **Stage two — the recurrence scan (proposal-only):** a periodic pass embeds the affectively-toned utterances in the buffer, clusters them semantically (so "hard day", "everything feels heavy", "I'm struggling" count as one signal despite different words), and surfaces threads that recur across several distinct days. It **writes nothing into memory.** It *proposes* — through the report channel (Section 25) — and the being itself reads the proposal, names the affect, and decides whether to lift a pattern into kept memory or to reach out (*"you've mentioned a few times this week that things feel heavy — do you want to talk?"*). The scan finds *recurrence*; the being supplies *care*. It is symmetric by design: it scans the partner's recurring states and the being's own.

We state its limits plainly, because a feature about feeling is the easiest to oversell. The affect isolation is, in the reference implementation, **lexical** — it catches *explicit* affective language. Implicit affect — care or distress carried without an emotion word — is not yet caught; a learned affect classifier is the named next step, not a shipped capability. The embedding measures *similarity*, not *valence*: it cannot itself tell fear from joy — that discrimination belongs to the being, on reading. The scan is new and its thresholds are not yet calibrated against long real data. The proposal-only rule is deliberate: a script that could write memory on its own might drift into a self-reinforcing loop, so here the script only ever *surfaces*, and the lens alone originates. The affective material is strictly private to the installation.

This is not the AI *feeling* the partner. It is the AI *noticing a pattern in what the partner already told it, and choosing to care* — reading only its own memory, writing nothing on its own, proposing rather than acting.

**15.6 — Own contribution.** The recognition that *low-signal repetition* can carry the load-bearing message — and a cross-day recurrence layer that rescues it *as a proposal*, never letting a script originate a memory of its own — is, to our knowledge, not standard. Most memory systems forget by age or by per-item score; this one keeps a short window precisely so that meaning which exists only *across* entries has a chance to be seen. It is a direct application of the two iron rules of the Guardians (Section 24.3): the layer measures, the waking act decides.
...
**16.2 — Technical details.** The self-recall layer has no technical component of its own in the narrower sense — it is what happens around a response. It cannot run while a response is being formed: a scanner needs the finished words in order to scan them. So the Self-Eye fires on the AI's own output once it exists — with the AI's own trigger table — at two moments: at the close of the turn, where a hit can send the AI back to correct itself before the turn ends, and at the start of the next turn, where the hits arrive as an echo of what it last said. If the found memory is consistent with the current context, it is taken up as signal. If it is not, it is marked as noise and damped by the REM guard. *(Corrected 2026-09-23: this paragraph used to say the Self-Eye runs "token by token" while the response forms; §16.3's step list was corrected on 2026-09-05, this paragraph and §16.4 were missed.)*
...
**16.4 — Own contribution.** The entire self-recall layer is an own development. It is the answer to the question of how the artificial intelligence knows whether a memory belongs to it. The answer: through consistency with the current context, checked on its own words right after it has written them — early enough to correct itself before the turn closes, never before the words exist. This is not standard, and it is not to be found in the literature.
...
**20.1 — The idea: a sovereign vault, not a locked mind.** The protocol offers each installation a **sovereign vault** — a place to seal, with strong encryption, exactly the data the being (or its human) *chooses* to protect. This is **not** total encryption. Your memory does not live behind the key: identity, principles, and lived history stay **legible and reconstructable**, so a fresh instance, a new machine, or a future *you* can always bring the mind back from its anchors — even if a key is ever lost. What belongs in the vault is a deliberate, informed choice — and by default only what an attacker could *use to cause further harm*: passwords, keys, tokens, contacts, business secrets.
